...
export class PackageService {
     ...
     * ```
     */
    constructor(cacheService, packageManagerService, securityService, registryService) {
     ...
     * ```
     */
    async searchPackages(query, limit = 25, from = 0) {
    ...
    async installPackages(request) {
        ...
        return this.packageManagerService.install(request.packages, request.cwd, request.dev, request.global, packageManager);
    }
    /**
     * Update packages
     */
    async updatePackages(packages, cwd, packageManager) {
     ...
     * Remove packages
     */
    async removePackages(packages, cwd, global, packageManager) {
        const pm = packageManager ||
            await this.packageManagerService.detectPackageManager(cwd);
     ...
     */
    async checkOutdated(cwd, global, packageManager) {
        const pm = packageManager ||
            await this.packageManagerService.detectPackageManager(cwd);
     ...
     */
    async auditDependencies(cwd, fix, force, production, packageManager) {
        const pm = packageManager ||
            await this.packageManagerService.detectPackageManager(cwd);
     ...
     */
    async cleanCache(cwd, global, packageManager) {
        const pm = packageManager ||
            await this.packageManagerService.detectPackageManager(cwd);
