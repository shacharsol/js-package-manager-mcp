...
export class PackageManagerService {
     ...
     * ```
     */
    async install(packages, cwd, dev, global, packageManager = PACKAGE_MANAGERS.NPM) {
     ...
     * ```
     */
    async update(packages, cwd, packageManager = PACKAGE_MANAGERS.NPM) {
        const startTime = Date.now();
     ...
     * ```
     */
    async remove(packages, cwd, global, packageManager = PACKAGE_MANAGERS.NPM) {
        const startTime = Date.now();
        try {
     ...
     * ```
     */
    async checkOutdated(cwd, global, packageManager = PACKAGE_MANAGERS.NPM) {
        const startTime = Date.now();
        try {
     ...
     * ```
     */
    async audit(cwd, fix, force, production, packageManager = PACKAGE_MANAGERS.NPM) {
        const startTime = Date.now();
        try {
     ...
     * ```
     */
    async cleanCache(cwd, global, packageManager = PACKAGE_MANAGERS.NPM) {
        const startTime = Date.now();
        try {
     ...
     */
    async executeCommand(packageManager, args, cwd, reject = true) {
     ...
     */
    buildInstallArgs(packages, dev, global, packageManager = PACKAGE_MANAGERS.NPM) {
        const args = [];
     ...
     */
    buildUpdateArgs(packages, packageManager = PACKAGE_MANAGERS.NPM) {
        const args = [];
        switch (packageManager) {
            case PACKAGE_MANAGERS.NPM:
                args.push('update');
     ...
     */
    buildRemoveArgs(packages, global, packageManager = PACKAGE_MANAGERS.NPM) {
        const args = [];
        switch (packageManager) {
            case PACKAGE_MANAGERS.NPM:
                args.push('uninstall');
     ...
     */
    buildOutdatedArgs(global, packageManager = PACKAGE_MANAGERS.NPM) {
        const args = ['outdated'];
        if (global) {
            switch (packageManager) {
                case PACKAGE_MANAGERS.NPM:
     ...
     */
    buildAuditArgs(fix, force, production, packageManager = PACKAGE_MANAGERS.NPM) {
        const args = ['audit'];
     ...
     */
    buildCleanArgs(global, packageManager = PACKAGE_MANAGERS.NPM) {
        switch (packageManager) {
            case PACKAGE_MANAGERS.NPM:
     ...
     */
    createErrorResult(error, packages, operation, packageManager, startTime) {
