...
export class RegistryService {
     ...
     * ```
     */
    constructor(registryUrl = URLS.NPM_REGISTRY, bundlephobiaUrl = URLS.BUNDLEPHOBIA_API, npmApiUrl = URLS.NPM_API) {
     ...
     * ```
     */
    async search(query, limit = 25, from = 0) {
        try {
            ...
            return data.objects?.map((item) => this.transformSearchResult(item)) || [];
        }
        catch (error) {
     ...
     */
    transformPackageManifest(manifest) {
        return {
            name: manifest.name,
            ...
            maintainers: manifest.maintainers?.map((m) => this.transformAuthor(m)),
